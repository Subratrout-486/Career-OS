...
from .control_plane import AgentMessage, ControlPlaneStore, TaskRecord, TaskStatus, utc_now


class AgentExecutor(Protocol):
    def __call__(self, *, objective: str, context: dict[str, Any], tools: list[str]) -> dict[str, Any]: ...
...
class MultiAgentRuntime:
    ...
    def run(self, *, agent_id: str, objective: str, context: dict[str, Any] | None = None, parent_task_id: str | None = None) -> AgentResult:
        ...
        task = self.store.create_task(TaskRecord(objective=objective, department=spec.department, agent_id=agent_id, parent_task_id=parent_task_id, payload={"runtime": "career-os-v2", "agent": agent_id, "context": context or {}}))
        task.status = TaskStatus.RUNNING
        self.store.update_task(task)
        self.harness.start_session(task.id, objective=objective, input_data=context or {})
        self.harness.start_step(task.id, step_index=0, input_data=context or {})
        try:
            ...
            return AgentResult(task.id, agent_id, TaskStatus.COMPLETED.value, output)
            ...
            return AgentResult(task.id, agent_id, task.status.value, {"error": str(exc)})
    ...
    def delegate(self, *, parent_task_id: str, agent_id: str, objective: str, context: dict[str, Any] | None = None) -> AgentResult:
        ...
        self.store.add_message(AgentMessage(task_id=parent_task_id, from_agent=self.store.get_task(parent_task_id).agent_id or "orchestrator", to_agent=agent_id, objective=objective, input=context or {}, evidence=[child.output], status=TaskStatus.COMPLETED if child.status == TaskStatus.COMPLETED.value else TaskStatus.FAILED))
        return child
