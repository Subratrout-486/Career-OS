...
Produce a tailored resume that prioritizes the strongest relevant confirmed evidence for this JD.
Do not invent. Do not keyword-stuff. Prefer Experience bullets over Skills when the evidence supports employer context.
...
CHALLENGE_PROMPT = """You are an independent red-team career reviewer for Career OS.
{truth_rules}
...
class AgentRuntime:
    ...
    async def _chat_gemini(self, system, user, *, json_mode, max_tokens):
        ...
        # Prefer configured model, then common resilient flash variants on 503.
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                if attempt < max_attempts:
                    user = (
                        user
                        + "\n\nPREVIOUS_OUTPUT_WAS_INVALID. Return ONLY the exact JSON schema requested."
                    )
                    continue
        raise StructuredOutputError(
            f"Failed to produce valid structured output after {max_attempts} attempts: {last_error}"
        ) from last_error
    ...
    async def analyze_fit(self, profile, job, evidence_pack=None):
        user = FIT_PROMPT.format(
            ...
            profile=profile,
            evidence=json.dumps(evidence_pack or [], default=str, indent=2),
        )
    ...
    async def tailor_resume(
        self, profile, job, fit, jd_analysis=None, evidence_pack=None
    ):
