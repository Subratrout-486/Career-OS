...
def classify_recruiter_review(notes: str | None, provider: str | None) -> RecruiterReview:
    ...
    independent_provider = provider_name.lower().startswith("deepseek")
    ...
    if not independent_provider:
        warnings.append("Independent reviewer provenance is not DeepSeek; treated as non-applying review.")
