...
def classify_recruiter_review(notes: str | None, provider: str | None) -> RecruiterReview:
    ...
    independent_provider = provider_name.lower().split(":", 1)[0] in {"anthropic", "gemini", "xai", "deepseek"}
    ...
    if not independent_provider:
        warnings.append("Independent reviewer provenance is not a verified independent provider; treated as non-applying review.")
