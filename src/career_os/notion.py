...
class NotionReviewQueue:
    ...
    async def _upload_file(self, path: str) -> str:
        ...
        async with httpx.AsyncClient(timeout=60) as client:
            create = await client.post(
                ...
                json={"mode": "single_part", "filename": file_path.name, "content_type": content_type},
            )
    ...
    def _ats_score(fit: dict, resume: dict) -> int:
        ...
        return round((sum(1 for keyword in keywords if keyword in resume_text) / len(keywords)) * 100)
    ...
    async def _create_resume_library_page(self, result: dict, upload_ids: list[tuple[str, str]]) -> str | None:
        ...
        files = [{"type": "file_upload", "file_upload": {"id": upload_id}, "name": filename} for filename, upload_id in upload_ids]
        version = "CareerOS-" + __import__("datetime").datetime.now(__import__("datetime").timezone.utc).strftime("%Y%m%d-%H%M%S")
        notes = (
        ...
        payload = {
            "parent": {"type": "data_source_id", "data_source_id": self.resume_library_data_source_id},
            "properties": {
                "Resume Name": {"title": [{"text": {"content": f"{job['company']} — {job['title']} — JD Tailored Resume"}}]},
    ...
    async def create_review_page(self, result: dict) -> tuple[str | None, str | None]:
        ...
        blocks += [
            ...
            self._paragraph("STATUS: READY_FOR_REVIEW — Review the JD, fit, challenger notes and resume before applying. Career OS never submits the application automatically."),
        ]
