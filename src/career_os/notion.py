...
class NotionReviewQueue:
    ...
    def _ats_score(fit: dict, resume: dict) -> int:
        ...
        matched = sum(1 for keyword in keywords if keyword in resume_text)
    ...
    async def _create_resume_library_page(self, result: dict, upload_ids: list[tuple[str, str]]) -> str | None:
        ...
        payload = {
            "parent": {"data_source_id": self.resume_library_data_source_id},
            "properties": {
    ...
    async def create_review_page(self, result: dict) -> tuple[str | None, str | None]:
        ...
        upload_ids = []
        for kind, path in (result.get("resume_files") or {}).items():
            if Path(path).exists():
        ...
        # Add downloadable file blocks to the review page as well as the Resume Library record.
        for filename, upload_id in upload_ids:
            blocks.append({
                "object": "block",
                "type": "file",
