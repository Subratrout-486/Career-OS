...
class NotionReviewQueue:
    ...
    async def _create_resume_library_page(
        self, result: dict, upload_ids: list[tuple[str, str]]
    ) -> str | None:
                ...
                "title": [
                    {
                        "type": "text",
                        "text": {
                            "content": f"{job['company']} — {job['title']} — JD Tailored Resume"
                        },
                    }
                ]
            },
            "Target Role": {
                "multi_select": [{"name": self._target_role(job["title"])}]
            },
        ...
        payload = {
            "parent": {
                "type": "data_source_id",
                "data_source_id": self.resume_library_data_source_id,
            },
        ...
        async with httpx.AsyncClient(timeout=60) as client:
            ...
            if response.is_error:
                raise RuntimeError(
                    f"Notion Resume Library create failed ({response.status_code}): "
    ...
    async def create_review_page(self, result: dict) -> tuple[str | None, str | None]:
        ...
        async with httpx.AsyncClient(timeout=60) as client:
            ...
            if response.is_error:
                raise RuntimeError(
                    f"Notion review page create failed ({response.status_code}): "
                    f"{response.text[:2000]}"
                )
