...
| Independent recruiter review | The challenger now prefers DeepSeek and falls back to xAI only when it remains independent from the resume-writing provider. Its notes are converted into a structured `PASS`, `REVISE`, `BLOCKED`, or `NOT_RUN` result. | Ambiguous, unavailable, or failed review is never represented as approval. |
| AUTO_APPLY gate | Browser eligibility now requires an explicit ATS pass, recruiter-review pass, design-QA pass, verified active job, and the existing verified browser/form/answer/resume facts. | The mode remains `REVIEW_REQUIRED` unless every deterministic prerequisite is present. |
...
| Execution manifest | `scripts/dispatch_manus_browser_tasks.py` accepts only explicit manifest records carrying all gate evidence, the exact artifact path, and a SHA-256 hash. | The dispatch script fails closed on an absent gate, human-controlled blocker, URL, record ID, artifact, or hash mismatch. |
...
| Check | Result |
|---|---|
| Focused application-mode, reviewer, design-QA, Manus runner, and manifest tests | Passed: 21 tests |
| Full project regression suite | Passed: 118 tests |
...
1. Add a `MANUS_API_KEY` repository secret with `create_task` scope.
2. Set the repository variable `CAREER_OS_EXECUTION_ENABLED` to `true`; it defaults to a safe no-op.
3. Optionally set `MANUS_BROWSER_AGENT_PROFILE`; the workflow defaults to `manus-1.6`.
4. Create a **verified browser-execution manifest**. Every record must contain the job URL and application ID, exact resume path and SHA-256 hash, an active-job signal, explicit ATS/recruiter/design-QA passes, verified form/answer/attachment signals, and no human-controlled blockers.
5. Run the GitHub workflow manually and choose an authenticated user browser for the created task if execution reaches a live form.
6. Review the task’s structured outcome and sync only an employer-confirmed `SUBMITTED` result to the application tracker.

This implementation deliberately does not create recurring unattended browser submissions. Discovery and analysis may continue on schedules, but live application execution remains a bounded, auditable, user-browser-mediated operation.
